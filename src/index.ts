/**
 * Guardian — Security Policy Engine for OpenClaw
 *
 * Bridges to the Python NexusEngine via persistent subprocess
...
const BRIDGE_SCRIPT = path.join(PLUGIN_DIR, "guardian_engine", "bridge.py");
...
function ensureProcess(): ChildProcess {
  ...
  proc.on("close", (code: number | null) => {
    ...
    for (const [rid, { reject }] of _pendingRequests) {
      reject(new Error(`Nexus bridge exited with code ${code}`));
    }
    ...
    try {
      ...
      setTimeout(() => {
        if (_pendingRequests.has(requestId)) {
          ...
          reject(new Error(`Nexus bridge timeout for command: ${command}`));
        }
      }, 30000);
...
export async function onStartup(context: {
  ...
  try {
    ...
    if (result.healthy) {
      ...
      console.log(
        `[guardian] Event bus ready: ${stats.total_events ?? 0} events, ${stats.consumer_groups ?? 0} groups`
      );
    } else {
      console.error(`[guardian] Health check failed: ${result.error}`);
    }
  } catch (err) {
    console.error(`[guardian] Startup error: ${(err as Error).message}`);
  }
}

export async function onShutdown(): Promise<void> {
  killProcess();
  console.log("[guardian] Event bus shut down");
}
